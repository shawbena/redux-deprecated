...
如果你的 `mapStateToProps` 接收两个参数，第二个参数将会是传递给  connected 组件的 props. 每次 conneccted 的组件接收新 props 时都会被调用。（第二个参数习惯上通常叫 `ownProps`）
...
如果  `mapDispatchToProps` 函数接收两个参数，第一个参数将会是 `dispatch` 传递给 connected 的组的 props 作为第二个参数，每当 connected 的组件接收新的 props 时，都将再次调用这个函数 (第二个参数习惯上通常叫 `ownProps`).
...
- [`connectOptions`] (Object) 如果指定的话，进一步自义 connector 的行为
